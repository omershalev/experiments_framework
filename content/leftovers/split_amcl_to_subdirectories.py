...
amcl_execution_dir = r'/home/omer/Desktop/apr_drift_noises_selected_instances'
output_dir = r'/home/omer/Desktop/apr_drift_noises_selected_instances_processed'
...
if __name__ == '__main__':
    ...
    odometry_noise_mu_x_experiments_mapping = {0.001: [], 0.002: [], 0.003: [], 0.004: [], 0.005: []}
    for experiment_name in os.listdir(amcl_execution_dir):
        ...
        odometry_noise_mu_x = summary['params']['odometry_noise_mu_x']
        ...
        if odometry_noise_mu_x != 0:
            odometry_noise_mu_x_experiments_mapping[odometry_noise_mu_x].append(experiment_name)
    ...
    for odometry_noise_mu_x, experiment_names in odometry_noise_mu_x_experiments_mapping.items():
        os.mkdir(os.path.join(output_dir, 'odometry_mu_x=%f' % odometry_noise_mu_x))
        for experiment_name in experiment_names:
            shutil.copytree(os.path.join(amcl_execution_dir, experiment_name), os.path.join(output_dir, 'odometry_mu_x=%f' % odometry_noise_mu_x, experiment_name))
            shutil.copy(os.path.join(amcl_execution_dir, experiment_name, 'error.png'), os.path.join(output_dir, 'odometry_mu_x=%f' % odometry_noise_mu_x, 'error_%s.png' % experiment_name))
    print ('end')
