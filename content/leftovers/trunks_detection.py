...
if __name__ == '__main__':
    ...
    for image_path in image_paths_list:
        ...
        # Get tree centroids
        centroids, rotated_centroids, _, _ = trunks_detection_old_cv.find_tree_centroids(cropped_image, correction_angle=orientation * (-1))
        if viz_mode:
        ...
        # Get essential grid
        essential_grid = trunks_detection_old_cv.get_essential_grid(grid_dim_x, grid_dim_y, shear, orientation, n=N)
        if viz_mode:
        ...
        grid = trunks_detection_old_cv.get_grid(grid_dim_x, grid_dim_y, translation, orientation, shear, n=N)
        ...
        opt = trunks_detection_old_cv._TrunksGridOptimization(grid_dim_x, grid_dim_y, translation, orientation, shear, sigma, cropped_image, n=N)
        ...
        optimized_grid = trunks_detection_old_cv.get_grid(optimized_grid_dim_x, optimized_grid_dim_y,
        ...
        scores_array_np = trunks_detection_old_cv.get_grid_scores_array(full_grid_np, image, optimized_sigma)

        pattern_np = np.ones((9, 10), dtype=np.int8)
        pattern_np[0:5, 0] = -1


        pattern_origin = trunks_detection_old_cv.fit_pattern_on_grid(scores_array_np, pattern_np)
